...
9. [Notification System & Discord Bot](#9-notification-system--discord-bot)
...
| Notification | Discord (Webhook + Bot) |
| IaC / Provisioning | Ansible |
| Manifest Mgmt | Kustomize + yq |
...
│
├── discord-bot/                   # Discord Bot สำหรับ Manual Sync
│   ├── bot.py                     #   Discord bot + HTTP API server
│   ├── Dockerfile                 #   Bot container image
│   ├── docker-compose.yaml        #   Bot deployment config
...
├── Jenkinsfile_Backend_Example.groovy   # ตัวอย่าง Jenkinsfile สำหรับ Backend
├── Jenkinsfile_Frontend_Example.groovy  # ตัวอย่าง Jenkinsfile สำหรับ Frontend
...
- `GITLAB_PAT` = `'GITLAB_PAT_CREDENTIALS'`
- `GITLAB_DEPLOY` = `'GITLAB_CREDENTIALS'`
...
## 9. Notification System & Discord Bot
...
### 9.2 Discord Bot (`discord-bot/bot.py`)

**วัตถุประสงค์**: รองรับ Manual Sync flow เมื่อ `autoSync=No`

**Architecture**:
- Discord.py bot + aiohttp HTTP server (port 5000)
- Jenkins pipeline → HTTP POST `/send-sync-prompt` → Bot → Discord embed with buttons

**Flow**:
1. Pipeline ตั้ง `autoSync=No`
2. Pipeline สร้าง ArgoCD app (ยังไม่ sync)
3. Pipeline ส่ง HTTP request ไป bot
4. Bot สร้าง Discord embed พร้อมปุ่ม **Proceed** / **Skip**
5. User กดปุ่ม → Bot ส่ง request ไป Jenkins input API
6. Pipeline ได้รับ signal → sync หรือ skip
7. ถ้า timeout 30 นาที → auto skip

**Security**:
- `ALLOWED_ROLE_ID` — จำกัดว่าใครกดปุ่มได้
- Validate `build_url` ต้องเริ่มด้วย `JENKINS_BASE_URL`
- ป้องกัน double-action (pending_urls set)

**Commands**:
- `/sync proceed` — reply to sync prompt message → approve sync
- `/sync skip` — reply to sync prompt message → skip sync
...
### 11.3 Manual Sync Flow (Discord Bot)

```
Pipeline              Discord Bot           Discord         User
   │                      │                    │              │
   ├─ autoSync=No         │                    │              │
   ├─ create app ─────────┤                    │              │
   ├─ POST /send-sync ───►│                    │              │
...
*เอกสารนี้ครอบคลุมการทำงานทั้งหมดของ Jenkins Shared Library เพื่อให้สามารถทำความเข้าใจ ดูแลรักษา และต่อยอดระบบได้*
